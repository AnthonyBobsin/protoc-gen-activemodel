...
import (
	...
	"github.com/AnthonyBobsin/protoc-gen-activemodel/ruby_types"
...
func ActiveModeler() *modelModule { return &modelModule{ModuleBase: &pgs.ModuleBase{}} }

func (m *modelModule) InitContext(c pgs.BuildContext) {
	...
	funcs := map[string]interface{}{
		...
		"rubyFieldType":            ruby_types.RubySetterFieldType,
	...
	m.tpl = template.Must(template.New("activemodel").Funcs(funcs).Parse(tpl))
	// m.serviceTpl = template.Must(template.New("rbiService").Funcs(funcs).Parse(serviceTpl))
}

func (m *modelModule) Name() string { return "activemodel" }

func (m *modelModule) Execute(targets map[string]pgs.File, pkgs map[string]pgs.Package) []pgs.Artifact {
...
func (m *modelModule) generate(f pgs.File) {
	op := strings.TrimSuffix(f.InputPath().String(), ".proto") + "_models_pb.rb"
	m.AddGeneratorTemplateFile(op, m.tpl, f)
}

// func (m *modelModule) generateServices(f pgs.File) {
...
// TODO(bobsin): handle enums
const tpl = `# frozen_string_literal: true
...
class {{ rubyMessageType . }}
  ...
  {{ range .Fields }}
  attribute :{{ .Name }}, {{ rubyFieldType . }}, description: ""{{ end }}
end
{{ end }}`

// const tpl = `# Code generated by protoc-gen-activemodel. DO NOT EDIT.
// # source: {{ .InputPath }}
// # typed: strict
// {{ range rubyModules . }}
// module {{ . }}; end{{ end }}
